...
function trgger(e){
  ...
  function previewFiles() {
     ...
     function readAndPreview(file) {
        if ( /\.(jpe?g|png|gif|mp4|webm|ogg|ogv|mp3|wav)$/i.test(file.name)) {	
          if (file.size < 30 * 1024 * 1024) { // MAX 10 mb
               ...
               reader.addEventListener("load", function () {
                 ...
                 tippy([image, video, audio]);
                   $(i).click(function(){
                     ...
                     console.log("xoas mej mayf ddi",$(div).attr('src'));
  ...
  async function uploadImg(){
    ...
    const media = [];
    $('*').each(function(){ 
        if ($(this).is('.Imgpreview > img') ) {
            const type = $(this).attr('data-tippy-content').split(".")[1].split("/")[0]
            // console.log(type.split("/")[0])
            const content = $(this).attr('src');
            media.push({
                media_type: type,
                media_url: content
            });
    ...
    if(res.status == 200){
        ...
        iziToast.success({title:'done',color:'green',progressBarColor:'green',message: 'Success, Your post has been upload',iconText: 'success_outline'});
        getFeed();
    }
    else
        iziToast.error({title:'Sorry',message: res.data.message,iconText: 'error_outline'});
